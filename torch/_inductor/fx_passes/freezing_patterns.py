...
def _register_int4_mm_fusion_patterns(device, val):
    ...
    # Register patterns for common group_size values
    for gs in [32, 64, 128, 256]:
        # 3-way fusion pattern
        def make_int4_pattern_three(group_size):
            def pattern(inp, w1, w2, w3, sz1, sz2, sz3):
                return (
        ...
        def make_int4_replacement_three(group_size):
            def replacement(inp, w1, w2, w3, sz1, sz2, sz3):
        ...
        # 2-way fusion pattern
        def make_int4_pattern_two(group_size):
            def pattern(inp, w1, w2, sz1, sz2):
                return (
                    aten._weight_int4pack_mm.default(inp, w1, group_size, sz1),
                    aten._weight_int4pack_mm.default(inp, w2, group_size, sz2),
                )
        ...
        def make_int4_replacement_two(group_size):
            def replacement(inp, w1, w2, sz1, sz2):
        ...
        register_replacement(
            make_int4_pattern_three(gs),
            make_int4_replacement_three(gs),
            [
                val(),
                int4_weight(),
                int4_weight(),
        ...
        register_replacement(
            make_int4_pattern_two(gs),
            make_int4_replacement_two(gs),
            [
                val(),
                int4_weight(),
                int4_weight(),
