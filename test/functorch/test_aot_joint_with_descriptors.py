...
def graph_capture(model, inputs, with_export):
    ...
    for gm1 in gm.modules():
        if isinstance(gm1, torch.fx.GraphModule):
            for n in gm1.graph.nodes:
                if "seq_nr" in n.meta:
                    if "custom" not in n.meta:
                        n.meta["custom"] = {}
                    n.meta["custom"]["seq_nr"] = n.meta["seq_nr"]
    gm.print_readable()
...
('call_function', 'getitem_1', {'mod_name': 'my_mod'})""",  # noqa: B950
        ...
        # TODO: the annotation on the backward graph is missing because the seq_nr on
        # invoke_subgraph's subgraphs are not working correctly. That need to be fixed
        inputs = (torch.randn(4, 3, requires_grad=True),)
        graph_module = graph_capture(model, inputs, with_export=True)
